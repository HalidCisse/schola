  ...
  def / = async.Planify {
    ...
    case req =>
      ...
      req match {
        ...
        case ContextPath(_, "/") & UserAgent(uA) & SessionKey(key) =>
        ...
        case POST(ContextPath(_, "/ChangePasswd")) & UserAgent(uA) & SessionKey(key) & Params(UserPasswd(_, passwd, Some(newPasswd), _)) =>
        ...
        case GET(ContextPath(_, "/Logout")) & UserAgent(uA) & SessionKey(key) =>
        ...
        case POST(ContextPath(_, "/Login")) & UserAgent(uA) & Params(UserPasswd(username, passwd, _, rememberMe)) => // TODO: recherche more on remember-me
