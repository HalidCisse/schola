...
trait Root { self: Plans =>
  ...
  private val log = org.clapper.avsl.Logger("oadmin.clients./")
  ...
  private val xHttp = dispatch.Http
              ...
              case GET(ContextPath(_, "/session")) & Jsonp.Optional(cb) =>
                ...
                withSession(req, key, uA) {
                  case Right(session) =>
                    ...
                    } getOrElse (SetCookies.discarding(SESSION_KEY, Flash.COOKIE_NAME) ~> JsonContent ~> ResponseString(cb wrap """{"error": true}""")))
