...
*/
  ...
  Mac = (function() {
    ...
    Mac.beforeSend = function(xhr, req) {
      var hdr;
      hdr = Mac.createHeader(session.access_token || session.key, xhr, req);
      return xhr.setRequestHeader('Authorization', hdr);
    };
    ...
    Mac.createHeader = function(id, xhr, req) {
      var mac, nonce, rs;
      nonce = Mac._genNonce(parseInt(session.issuedTime));
      rs = Mac.toReqString(nonce, req.type, req.url, document.location.hostname, document.location.port || 80);
  ...
  Session = (function(_super) {
    ...
    Session.prototype.contructor = function(app) {
      this.app = app;
      Session.__super__.contructor.apply(this, arguments);
    ...
    Session.prototype.getLoginStatus = function() {
      ...
      doCheckSession = function() {
        ...
        x.success(function(s) {
          ...
          if (s.error) {
            return _this.app.delay(function() {
              return _this.app.navigate('/');
            });
          }
          _this.app.delay(function() {
          ...
          _this.trigger('session.loggedin');
          return false;
        });
        return x.error(function() {
          _this.log("getLoginStatus failure", arguments);
          delete _this.key;
          delete _this.secret;
          delete _this.clientId;
          delete _this.issuedTime;
          delete _this.user;
          delete _this.lastAccessTime;
          _this.expiresIn && delete _this.expiresIn;
          _this.refreshExpiresIn && delete _this.refreshExpiresIn;
          _this.refresh && delete _this.refresh;
          delete _this.userAgent;
          delete _this.hasRole;
          delete _this.hasPermission;
          delete _this.scopes;
          $.ajaxSetup({
            beforeSend: null
          });
          clearInterval(_this._refreshInterval);
          _this.trigger('session.loggedout');
          return false;
        });
      };
      doCheckSession();
      return this._refreshInterval = setInterval(doCheckSession, 15000);
    };
  ...
  users = (function() {
    ...
    users.upsertUser = function(spec) {
      return $.ajax({
        type: spec.id ? 'PUT' : 'POST',
        url: "/api/v1/users" + (spec.id ? '/' + spec.id : ''),
        dataType: 'json',
        contentType: 'application/json; charset=UTF-8',
        data: JSON.stringify(spec)
      });
