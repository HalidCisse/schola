  ...
  "OAuth2 requests for grant type password" should {
    ...
    "accept our user's password credentials" in {
      val (head, body) = http(token << Map(
        ...
        "username" -> owner.id,
        "password" -> sPassword) <:< Map("User-Agent" -> "Chrome") >+ { r => (r >:> { h => r }, r as_str) })
      json(body) { map =>
    ...
    "have appropriate Cache-Control and Pragma headers" in {
      val headers = http(token << Map(
        ...
        "client_secret" -> client.secret,
        "username" -> owner.id,
        "password" -> sPassword) <:< Map("User-Agent" -> "Chrome") >:> { h => h })
    ...
    "refresh token for a valid access_token" in {
      ...
      val req = token << Map(
        ...
        "client_secret" -> client.secret,
        "username" -> owner.id,
        "password" -> sPassword) <:< Map("User-Agent" -> "Chrome") as_! (client.id, client.secret)
      ...
      json(ares) { map =>
        ...
        val rres = http(token << Map(
          ...
          "client_id" -> client.id,
          "client_secret" -> client.secret,
          "refresh_token" -> map("refresh_token").toString) <:< Map("User-Agent" -> "Chrome") as_str)
        json(rres) { map2 =>
