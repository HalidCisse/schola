...
import schola.oadmin._, domain._, cl.SessionSupport
    ...
    implicit request =>
      ...
      getToken match {
          ...
          { Enumerator.fromFile(request.body.file) |>>> Iteratee.consume[Array[Byte]]() } flatMap {
            bytes =>
              ...
              use[SessionSupport].setAvatar(sessionKey, userAgent, filename, request.contentType, bytes) map {
                success => Ok(Json.obj("success" -> success))
              } recover {
                case ex: Throwable =>
        ...
        case _ =>
          ...
          scala.concurrent.Future.successful(Ok(Json.obj("success" -> false)))
      }
  }
      ...
      getToken match {
        ...
        case Some(sessionKey) =>
          ...
          use[SessionSupport].getAvatar(sessionKey, userAgent) map {
            case domain.AvatarInfo(filename, contentType, data, base64) => Ok(Json.obj("filename" -> filename, "contentType" -> contentType, "data" -> data, "base64" -> base64))
          } recover {
            case ex: Throwable => NotFound
          }
      ...
      getToken match {
        ...
        case Some(sessionKey) =>
          ...
          use[SessionSupport].purgeAvatar(sessionKey, userAgent) map {
            case success => Ok(Json.obj("success" -> success))
          } recover {
            case ex: Throwable => Ok(Json.obj("success" -> false))
          }
